...
OpenCode 使用 Turborepo 管理 12+ 个子包，类似 Java 的多模块 Maven 项目。
...
| ID | `id/` | ULID ID 生成器 |
| Flag | `flag/` | 功能开关 |
| auth | `auth/` | 认证与授权管理（Provider Token、OAuth） | Spring Security |
| control-plane | `control-plane/` | 控制平面：Server 路由、健康检查、CORS | API Gateway |
| project | `project/` | 项目配置检测（.opencode/opencode.json） | Spring Boot Config |
| env | `env/` | 环境变量解析与校验 | @Value / @ConfigurationProperties |
| command | `command/` | CLI 命令注册与参数解析 | CLI Framework |

> 仅列出核心模块，完整模块列表见源码 `src/` 目录（共 44 个模块）。
...
| 配置 | opencode.json / config.json | application.yml | JSON 格式（全局 config.json + 项目 opencode.json） |
