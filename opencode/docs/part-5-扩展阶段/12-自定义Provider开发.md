...
ProviderService 是 Provider 注册与查询的核心接口，负责管理所有 AI 提供商的生命周期。核心方法包括：`list()`（列出所有 Provider）、`getProvider(id)`（获取 Provider 实例）、`getModel(model)`（获取模型配置）、`getLanguage()`（获取编程语言）、`closest()`（最相似模型匹配）、`defaultModel()`（默认模型选择）。

> 完整的 ProviderService 接口定义、数据流图和注册流程请参见 [04-Provider 适配与 API 调用](../part-2-推理阶段/04-Provider适配与API调用.md)。
...
Provider 的注册流程涉及多个数据源的合并：内置模型数据库 → 环境变量 → opencode.json 配置 → API Key → 插件认证加载器 → 插件模型扩展，最终经过过滤（disabled/alpha/deprecated/blacklist）得到可用的 Provider 列表。

> 完整的 Provider 注册流程、数据源优先级和合并算法请参见 [04-Provider 适配与 API 调用](../part-2-推理阶段/04-Provider适配与API调用.md)。

### 3.2 数据源优先级

数据源按优先级从高到低：opencode.json 配置 → 插件 auth.loader → custom() 特殊处理 → Auth API Keys → 环境变量 → ModelsDev 内置数据库，通过 `mergeDeep()` 深度合并为最终 Provider 配置。

> 完整的数据源优先级说明请参见 [04-Provider 适配与 API 调用](../part-2-推理阶段/04-Provider适配与API调用.md)。
